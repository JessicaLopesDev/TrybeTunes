...
describe('1 - Crie um formulário para identificação', () => {
  ...
  it('Será validado se ao navegar para a rota /, o input e o botão especificados estão presentes', async () => {
    renderPath('/');
  ...
  it('Será validado se o botão só é habilitado se o input de nome tiver 3 ou mais caracteres', async () => {
    renderPath('/');
    ...
    const loginNameInput: HTMLInputElement =
      screen.getByTestId('login-name-input');
  ...
  it('Será validado se ao clicar no botão habilitado, a função createUser da userAPI é chamada', async () => {
  ...
  it('Será validado se ao clicar no botão, a mensagem Carregando... é exibida e a informação do usuário é salva corretamente', async () => {
    renderPath('/');
